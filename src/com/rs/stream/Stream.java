...
public abstract class Stream {
	...
	public final void xteaDecrypt(int keys[], int start, int end) {
		...
		for (int j1 = 0; j1 < i1; j1++) {
			offset += 4;
			int k1 = ((0xff & buffer[-3 + offset]) << 16) + ((((0xff & buffer[-4 + offset]) << 24) + ((buffer[-2 + offset] & 0xff) << 8)) + (buffer[-1 + offset] & 0xff));
			offset += 4;
			int l1 = ((0xff & buffer[-3 + offset]) << 16) + ((((0xff & buffer[-4 + offset]) << 24) + ((buffer[-2 + offset] & 0xff) << 8)) + (buffer[-1 + offset] & 0xff));
			int sum = 0xc6ef3720;
			for (int k2 = 32; k2-- > 0;) {
