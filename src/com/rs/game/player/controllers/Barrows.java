...
public final class Barrows extends Controller {
    ...
    Item[] BARROW_REWARDS = { new Item(4708, 1), new Item(4710, 1),
	    ...
	    new Item(25652, 1), new Item(25672, 1) };
    ...
    public void drop(Item item) {
    	Item dropItem = new Item(item.getId(), Utils.random((int) (item.getDefinitions().isStackable() ? 
    			item.getAmount() * player.getDropRate() : item.getAmount())) + 1);
    ...
    public void sendReward() {
		...
		if (Defenders.getCurrentTier(player, 0) && Utils.random(74-defender) == 0) {
		...
		drop(new Item(995, Utils.random(50000, 1000000))); // money reward at least always
    }
