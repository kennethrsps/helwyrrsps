...
public class TreasureTrails implements Serializable {
		...
		if (requiredItems.length > 0) {
			for (Item item : requiredItems) {
				...
				if (slot == Equipment.SLOT_RING && item.getId() == 20922) {
					if (!player.getEquipment().containsOneItem(new int[]
	...
	private static enum ClueDetails {
		...
		{ 20 }, new Item[] {}, "Panic in the heart of", "the Haunted Woods.", "Beware of double agents!", "Have no items equipped when you do."),
		EMOTE_P_2(EASY, EMOTE, TILE, 55250348, new int[]
		{ 20 }, new Item[]
