...
public class XPLamps {
	...
	public static final int[] SELECTABLE_XP_LAMPS = new int[]
	{ 23713, 23714, 23715, 23716 };
	public static final int[] SELECTABLE_XP_LAMPS_TYPES = new int[]
	{ LAMP_SMALL, LAMP_MEDIUM, LAMP_BIG, LAMP_HUGE };
	public static final int[][] SKILL_LAMPS = new int[][]
	{
		...
		player.getDialogueManager().startDialogue(new Dialogue() {
			...
			public void run(int interfaceId, int componentId) {
				if (componentId >= 13 && componentId <= 37) {
					...
					double exp = getExp(player.getSkills().getLevelForXp(selectedSkill), id);
					player.getSkills().addXp(selectedSkill, exp);
					player.closeInterfaces();
					player.getDialogueManager().startDialogue("SimpleMessage", "<col=0000ff>Your wish has been granted!</col>", "You have been awarded " + Utils.getFormattedNumber(exp, ',') + " XP in " + Skills.SKILL_NAME[selectedSkill] + "!");
				}
			}
	...
	private static void openSkillDialog(Player player, final int slot, final int id) {
		...
		player.getDialogueManager().startDialogue(new Dialogue() {
			@Override
			public void start() {
				sendOptionsDialogue("Gain <col=ff0000>" + Skills.SKILL_NAME[skillId] + "</col> experience?", "Yes", "No");
			}
			...
			public void run(int interfaceId, int componentId) {
				...
				player.getDialogueManager().startDialogue("SimpleMessage", "<col=0000ff>Your wish has been granted!</col>", "You have been awarded EXP in " + Skills.SKILL_NAME[skillId] + "!");
			}
