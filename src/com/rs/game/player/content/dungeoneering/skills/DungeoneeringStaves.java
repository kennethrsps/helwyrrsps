...
import com.rs.game.player.dialogue.impl.dungeoneering.DungRunecraftingD;
	...
	public boolean start(Player player) {
		...
		if (player.getSkills().getLevel(Skills.RUNECRAFTING) < levelReq) {
			player.getDialogueManager().startDialogue("SimpleMessage",
					"You need a Runecrafting level of " + levelReq + " in order to imbue this stave.");
			return false;
		}
		int staves = getUsableStaves(player, index);
		if (staves == 0) {
			player.getPackets().sendGameMessage("You don't have enough materials.");
			return false;
		}
		if (cycles < staves)
			cycles = staves;
