...
public class DungeoneeringConsumables {
	...
	public static enum Effect {
		...
		ONION_MESSAGE(1957) {
			@Override
			public void activateEffect(Player player) {
				player.getPackets().sendGameMessage("It hurts to see a grown "
						+ (player.getGlobalPlayerUpdater().isMale() ? "male" : "female") + " cry.");
			}
		},
	...
	public static boolean eat(Player player, int slot, Item item) {
		...
		player.setNextAnimation((player.getCombatDefinitions().isCombatStance()) ? COMBAT_EAT_ANIM : EAT_ANIM);
