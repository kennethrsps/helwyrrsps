...
public class House implements Serializable {
	...
	public void openRoomCreationMenu(int roomX, int roomY, int plane) {
		...
		if (room != null) {
			...
			for (int index = 0; index < HouseConstants.Room.values().length - 2; index++) {
				...
				if (player.getSkills().getLevel(Skills.CONSTRUCTION) >= refRoom.getLevel() && player.hasMoney(refRoom.getPrice()))
	...
	public void createRoom(RoomReference room) {
		if (!loaded)
			return;
		if (!player.hasMoney(room.room.getPrice())) { //better double check if somehow u manage to drop money
			player.sendMessage("You don't have enough coins to build this room.");
			return;
		}
	...
	public void openBuildInterface(WorldObject object, final Builds build) {
		...
		for (int index = 0; index < build.getPieces().length; index++) {
			...
			player.getVarBitManager().sendVar(1485 + index, (player.getSkills().getLevel(Skills.CONSTRUCTION) >= piece.getLevel() 
					&& player.getInventory().containsItems(piece.getRequirements()) || player.isDeveloper()) ? 1 : 0);
		}
	...
	public void build(int slot) {
		...
		if (build == null || object == null) // safety check
			return;
		if (build.getPieces().length > 3) { //this is safe now, because we checked build for nulls!
			if (slot == 2)
				slot = 1;
			else if (slot == 4)
				slot = 2;
	...
	public static void enterHouse(Player player, String displayname) {
		...
		if (!player.withinDistance(owner.getHouse().location.getTile(), 16)) {
			player.sendMessage("This house is at " + Utils.formatPlayerNameForDisplay(owner.getHouse().location.name()) + ".");
			return;
		}
	...
	public boolean joinHouse(final Player player) {
		if (!isOwner(player)) { //not owner
			if (!isOwnerInside() || !loaded) {
				player.sendMessage("That player is offline, or has privacy mode enabled."); //TODO message
				return false;
			}
			if (buildMode) {
