...
public final class PlayerLook {
	...
	if (buttonId == 6)
		...
		player.getGlobalPlayerUpdater().setHairStyle(
			(int) ClientScriptMap.getMap(
				player.getGlobalPlayerUpdater().isMale() ? 2339 : 2342)
				.getKeyForValue(slotId / 2));
	    else if (player.getGlobalPlayerUpdater().isMale())
		player.getGlobalPlayerUpdater().setBeardStyle(
			ClientScriptMap.getMap(703).getIntValue(slotId / 2));
	} else if (buttonId == 16)
	    player.getGlobalPlayerUpdater().setHairColor(
	...
	if (buttonId == 14 || buttonId == 16 || buttonId == 15
		...
		return;
	    if (male == player.getGlobalPlayerUpdater().isMale()
		    && skin == player.getGlobalPlayerUpdater().getSkinColor())
		player.getDialogueManager().startDialogue("MakeOverMage", 2676,
			1);
	    else {
		player.getDialogueManager().startDialogue("MakeOverMage", 2676,
			2);
		if (player.getGlobalPlayerUpdater().isMale() != male) {
		    ...
		    if (male)
			player.getGlobalPlayerUpdater().resetAppearence();
		    else
			player.getGlobalPlayerUpdater().female();
		}
		player.getGlobalPlayerUpdater().setSkinColor(skin);
	...
	if (buttonId == 6)
	    ...
	    if (ClientScriptMap.getMap(
		    player.getGlobalPlayerUpdater().isMale() ? 690 : 1591)
		    .getKeyForValue(player.getGlobalPlayerUpdater().getTopStyle()) >= 32) {
		...
		player.getGlobalPlayerUpdater().setTopStyle(
			ClientScriptMap.getMap(
				player.getGlobalPlayerUpdater().isMale() ? 690 : 1591)
				.getIntValue(slotId / 2));
		if (!player.getGlobalPlayerUpdater().isMale())
		    player.getGlobalPlayerUpdater().setBeardStyle(
			    player.getGlobalPlayerUpdater().getTopStyle());
		player.getGlobalPlayerUpdater().setArmsStyle(
			player.getGlobalPlayerUpdater().isMale() ? 26 : 65); // default
		player.getGlobalPlayerUpdater().setWristsStyle(
			player.getGlobalPlayerUpdater().isMale() ? 34 : 68); // default
	    } else if (stage == 1) // arms
		player.getGlobalPlayerUpdater().setArmsStyle(
			ClientScriptMap.getMap(
				player.getGlobalPlayerUpdater().isMale() ? 711 : 693)
				.getIntValue(slotId / 2));
	    else if (stage == 2) // wrists
		player.getGlobalPlayerUpdater().setWristsStyle(
			ClientScriptMap.getMap(751).getIntValue(slotId / 2));
	    else
		player.getGlobalPlayerUpdater().setLegsStyle(
			ClientScriptMap.getMap(
				player.getGlobalPlayerUpdater().isMale() ? 1586 : 1607)
				.getIntValue(slotId / 2));
		...
		player.getGlobalPlayerUpdater().setTopColor(
			ClientScriptMap.getMap(3282).getIntValue(slotId / 2));
	    else if (stage == 3)
		player.getGlobalPlayerUpdater().setLegsColor(
	...
	player.setCloseInterfacesEvent(new Runnable() {
		...
		player.getDialogueManager().startDialogue(
			...
			598,
			"An excellent choise, "
				+ (player.getGlobalPlayerUpdater().isMale() ? "sir"
					: "lady") + ".");
		player.setNextAnimation(new Animation(-1));
		player.getGlobalPlayerUpdater().getAppeareanceData();
	...
	player.getPackets().sendConfigByFile(6098,
		player.getGlobalPlayerUpdater().isMale() ? 0 : 1);
	player.getPackets().sendConfigByFile(6099,
		player.getGlobalPlayerUpdater().getSkinColor());
	player.getTemporaryAttributtes().put("MageMakeOverGender",
		player.getGlobalPlayerUpdater().isMale());
