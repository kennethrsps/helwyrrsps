...
public class BobBarter {
	...
	for (int i = 0; i < player.getInventory().getItemsContainerSize(); i++) {
	    ...
	    if(item.getName().contains("potion") || !item.getName().contains("flask")) {
	    for (int a = 1; a < 5; a++) {
	    ...
	    if(item.getName().contains("flask")) {
		    for (int a = 1; a < 7; a++) {
			for (int f = 23194; f < 26755; f++) {
			    if (item.getName().contains("(" + a + ")")
				    && item.getName().equals(
					    ItemDefinitions.getItemDefinitions(f)
						    .getName())) {
	...
	for (int i = 0; i < potsList.size(); i++) {
	    ...
	    if(itemName.contains("potion") || !itemName.contains("flask")) {
	    ...
	    if(itemName.contains("flask")) {
		    fullVials = totalDoses / 6;
		    partDoses = totalDoses % 6;
		    for (int x = 1; x < 7; x++) {
			if (itemName.contains("(" + x + ")")) {
			    if (checkIfNoted(itemName) == false) {
				emptyVialId = -1;
				fullVial = itemName.replace(Integer.toString(x), "6");
				partVial = itemName.replace(Integer.toString(x),
					Integer.toString(partDoses));
			    } else {
				emptyVialId = -1;
				fullVial = itemName.substring(1, itemName.length())
					.replace(Integer.toString(x), "6");
				partVial = itemName.substring(1, itemName.length())
					.replace(Integer.toString(x),
						Integer.toString(partDoses));
