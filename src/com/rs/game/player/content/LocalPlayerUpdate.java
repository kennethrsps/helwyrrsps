...
//	private byte[] slotFlags;
	...
	public boolean needAppearenceUpdate(int index, byte[] hash) {
		if (totalRenderDataSentLength > ((Protocol.SV_PACKET_SIZE_LIMIT - 500) / 2)
				|| hash == null)
			return false;
	...
	private void processOutsidePlayers(OutputStream stream, OutputStream updateBlockData/*
		...
		for (int i = 0; i < outPlayersIndexesCount; i++) {
			...
			if (needsAdd(p)) {
				...
				boolean needAppearenceUpdate = needAppearenceUpdate(p.getIndex(), p.getGlobalPlayerUpdater().getMD5AppeareanceDataHash());
			...
			if (needsRemove(p)) {
				...
				if (p.hasTeleported()) {
					...
					for (int i2 = i + 1; i2 < localPlayersIndexesCount; i2++) {
						...
						if (needsRemove(p2) || p2.hasTeleported() || p2.getNextWalkDirection() != -1 || (p2.needMasksUpdate() || needAppearenceUpdate(p2.getIndex(), p2.getGlobalPlayerUpdater().getMD5AppeareanceDataHash())))
							break;
						skip++;
					}
					skipPlayers(stream, skip);
	...
	private void appendUpdateBlock(Player p, OutputStream data,
		...
		if (added || (p.getNextFaceWorldTile() != null 
				&& p.getNextRunDirection() == -1 && p.getNextWalkDirection() == -1))
			maskData |= 0x40;
	...
	private void applyAppearanceMask(Player p, OutputStream data) {
		...
		cachedAppearencesHashes[p.getIndex()] = p.getGlobalPlayerUpdater()
