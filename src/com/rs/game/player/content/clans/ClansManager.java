...
public class ClansManager {
	...
	player.getPackets().sendRunScript(4319); // blank wot
    }
	...
	for (int i = 0; i < manager.clan.getMottifColors().length; i++)
	    player.getPackets().sendConfig(2094 + i,
	...
	synchronized (this) {
	    player.getPackets().sendConfig(2094 + part,
    ...
    public static void invite(Player player, Player p2) {
		...
		synchronized (manager) {
		    if (!manager.hasRankToInvite(player)) {
		    	player.sendMessage("You don't have the permission to invite other players.");
		    	return;
		    }
		    if (manager.getClan().getBannedUsers().contains(p2.getUsername())) {
		    	player.sendMessage("This player has been banned from this clan chat channel.");
		    	return;
		    }
		    if (manager.clan.getMembers().size() >= Clan.MAX_MEMBERS) {
		    	player.sendMessage("Clan chat channels can't have over 500 members.");
		    }
		}
		...
		player.sendMessage("Sending " + p2.getDisplayName() + " an invitation to join your clan..");
		p2.getPackets().sendClanInviteMessage(player);
		p2.getTemporaryAttributtes().put("claninvite", Boolean.TRUE);
	}
    
    public static void declineInvitation(Player player, Player p2) {
		p2.sendMessage("Player: " + p2.getDisplayName() + " has declined your Clan Invitation.");
		player.getTemporaryAttributtes().remove("claninvite", Boolean.TRUE);
		player.sendMessage("You have declined "+p2.getDisplayName()+"'s Clan Invitation.");
	}
}
