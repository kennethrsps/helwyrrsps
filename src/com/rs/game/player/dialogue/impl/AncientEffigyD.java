...
public class AncientEffigyD extends Dialogue {
	...
	public void run(int interfaceId, int componentId) {
		if (stage == -1) {
			sendDialogue(new String[]
			{ "Images from your experiences of " + AncientEffigy.getMessage(skill1), "fill your mind." });
			stage = 0;
		} else if (stage == 0) {
			...
			if (player.getSkills().getLevelForXp(skill) < AncientEffigy.getRequiredLevel(itemId)) {
				...
				player.getPackets().sendGameMessage("You require at lest level " + AncientEffigy.getRequiredLevel(itemId) + " " + Skills.SKILL_NAME[skill] + " to investigate the ancient effigy further.");
				stage = -2;
			} else {
				sendDialogue(new String[]
			...
			sendDialogue(new String[]
			{ "The ancient effigy glows briefly; it seems changed", "somehow and no longer responds to the same memories", "as before." });
			stage = 3;
		} else if (stage == 3) {
			sendDialogue(new String[]
			{ "A sudden bolt of inspiration flashes through your mind,", "revealing new insight into your experiences!" });
			stage = -2;
		} else {
			end();
		}
