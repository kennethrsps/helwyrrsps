...
public class TeplinMacaganD extends Dialogue {
	...
	public void start() {
		...
		case 1:
			stage = -1;
			sendOptionsDialogue(DEFAULT_OPTIONS_TITLE, "Can you convert my flasks into normal vials for me?",
					"Can you decant my potions into flasks, please?",
					"Can you decant my flasks back into 6 dosed flasks, please?", "I've got to go.");
			break;
		case 2:
			stage = 0;
	...
	public void run(int interfaceId, int componentId) {
		...
		case -1:
			...
			case OPTION_2:
			case OPTION_3:
				player.getDialogueManager().startDialogue("TeplinMacaganD", npcId,
						componentId == OPTION_1 ? 2 : componentId == OPTION_2 ? 3 : 4);
				break;
			default:
				end();
				break;
	...
	private void startDecantingProcess() {
		...
		case 2:
			...
			for (Pot flask : Pot.values()) {
				int flaskDoses = 0;
				if (flask == null || !flask.isFlask() || flask.ordinal() >= Pot.GRAND_STRENGTH_POTION.ordinal())
					continue;
				for (int i = flask.getId().length - 1; i >= 0; i--) {
					int id = flask.getId()[i];
					if (player.getInventory().containsItem(id, 1)) {
						flaskDoses += player.getInventory().getNumerOf(id) * (flask.getId().length - i);
					}
					if (player.getInventory().containsItem(ItemDefinitions.getItemDefinitions(id).getCertId(), 1)) {
						flaskDoses += player.getInventory().getNumerOf(
								ItemDefinitions.getItemDefinitions(id).getCertId()) * (flask.getId().length - i);
					}
				}
				...
				int dosesRemoved = 0;
				for (int i = 6; i > 0; i--) {
					while (dosesRemoved <= flaskDoses && (player.getInventory().containsItem(flask.getIdForDoses(i), 1)
							|| player.getInventory().containsItem(
									ItemDefinitions.getItemDefinitions(flask.getIdForDoses(i)).getCertId(), 1))) {
						...
						if (player.getInventory().containsItem(
								ItemDefinitions.getItemDefinitions(flask.getIdForDoses(i)).getCertId(), 1)) {
							player.getInventory().deleteItem(new Item(
									ItemDefinitions.getItemDefinitions(flask.getIdForDoses(i)).getCertId(), 1));
							dosesRemoved += i;
						}
					}
				...
				for (int i = 4; i > 0; i--) {
					...
					if (amount == 0 || dosesRemoved <= 0)
						continue;
					Item item = new Item(ItemDefinitions.getItemDefinitions(potion.getIdForDoses(i)).getCertId(),
							amount);
			...
			player.getDialogueManager().startDialogue("SimpleNPCMessage", npcId, potionsToAdd.size() > 0
					? "Here you go. That's the best i can do!"
		...
		case 3:
			...
			int maxDoses = (player.getInventory().getNumerOf(23191) * 6)
					+ (player.getInventory().getNumerOf(23191 + 1) * 6);
			for (Pot potion : Pot.values()) {
				int doses = 0;
				if (potion == null || !potion.isPotion() || potion.ordinal() >= Pot.GRAND_STRENGTH_POTION.ordinal())
					continue;
				for (int i = 3; i >= 0; i--) {
					int id = potion.getId()[i];
					if (player.getInventory().containsItem(id, 1)) {
						doses += player.getInventory().getNumerOf(id) * (potion.getId().length - i);
					}
					if (player.getInventory().containsItem(ItemDefinitions.getItemDefinitions(id).getCertId(), 1)) {
						doses += player.getInventory().getNumerOf(ItemDefinitions.getItemDefinitions(id).getCertId())
				...
				int dosesRemoved = 0;
				for (int i = 4; i > 0; i--) {
					while (dosesRemoved <= doses && (player.getInventory().containsItem(potion.getIdForDoses(i), 1)
							|| player.getInventory().containsItem(
									ItemDefinitions.getItemDefinitions(potion.getIdForDoses(i)).getCertId(), 1))) {
						if (player.getInventory().containsItem(potion.getIdForDoses(i), 1)) {
							player.getInventory().deleteItem(new com.rs.game.item.Item(potion.getIdForDoses(i), 1));
				...
				for (int i = 6; i > 0; i--) {
					...
					if (amount == 0 || dosesRemoved <= 0)
						continue;
					Item item = new Item(ItemDefinitions.getItemDefinitions(flask.getIdForDoses(i)).getCertId(),
							amount);
					if (player.getInventory().getFreeSlots() > 0) {
						potionsToAdd.add(item);
						dosesRemoved = (dosesRemoved % i);
