		...
		if (stage == -1) {
			...
			if (cost == -1)
				sendNPCDialogue(npcId, 9827,
						"This boat? Why this boat takes you to " + ship.getFixedName(returning) + ".");
			else
				sendNPCDialogue(npcId, 9827, "This boat? Why this boat takes you to " + ship.getFixedName(returning)
						+ ", for a small fee of " + cost + " coins.");
		} else if (stage == 2) {
			if (cost != -1)
				sendOptionsDialogue("Pay the price of " + cost + " coins?", "Yes, board the ship.",
						"I can't affored that!");
			else
